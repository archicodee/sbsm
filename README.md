...
**Настройки параллельной проверки** — управляют скоростью и точностью проверки прокси:

| Параметр | По умолчанию | Описание |
|----------|:------------:|----------|
| **Batch Size** | 10 | Количество прокси, проверяемых одновременно (параллельно). Значение 1 — последовательная проверка. |
| **Timeout** | 5000ms | Таймаут на один прокси при проверке через clash_api. Если прокси не ответил за это время — помечается как недоступный. |

> **Как работает параллельная проверка:**
> - Скрипт запускает отдельный экземпляр sing-box со всеми прокси из базы и использует clash_api для измерения задержки.
> - Прокси проверяются **батчами** (группами) по `batch_size` штук одновременно — это даёт значительное ускорение.
> - При `batch_size=10` и 130 прокси проверка занимает ~66с вместо ~279с при последовательной (**4.2x ускорение**).
> - Слишком большой `batch_size` (20+) может вызвать перегрузку роутера или таймауты. Рекомендуемое значение: **10**.
> - Слишком маленький `timeout` (1000-2000ms) может дать ложные срабатывания — прокси с реальной задержкой 1500ms будут помечены как недоступные. Рекомендуемое значение: **5000ms**.
> - Настройки доступны в меню: **Settings → Checking Mode → 7. Batch Size / 8. Timeout**, или через CLI: `batch_size [N]` / `check_timeout [N]`.
